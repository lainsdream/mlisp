...
— это намеренно: если бы хелпер исполнял бинарник прямо из Homebrew-префикса
(`/usr/local/bin/tun2socks`, обычно принадлежит текущему пользователю или
группе `admin`), то любой, кто может подменить этот файл, получал бы
код-выполнение от root через `sudo -n lisp-vpn-priv start-tun`. Поэтому
исполняемый tun2socks — это отдельная, скопированная под root копия:
...
Если Homebrew обновит `tun2socks`, нужно **повторить последнюю команду
вручную** — root-копия не следит за оригиналом сама.
